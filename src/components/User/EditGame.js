...
import {editGame, clearCreateGameError, clearGameEditError} from './../../redux/actions';
...
class EditGame extends Component { 
    ...
    componentWillMount(){
        const {sides, category, description, stake_end_time, start_time, completion_time, minimum_stake} = this.props.mySingleGame;
        
        if(this.props.mySingleGame.category){
            this.setState({
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(nextProps.mySingleGame.category){
            this.setState({
                category: category.id,
                description,
                start_time,
        ...
        // this.setState({category: nextProps.mySingleGame.category.id});
    }
    ...
    editMyGame(){
        this.props.editGame(this.props.mySingleGame.id, this.state);
    }


    renderGameEditError(){
        
        if(this.props.gameEditError.length > 0){
            
            setTimeout(() => this.props.clearGameEditError(), 3000);
            
                return (
                    <View style={{backgroundColor: PINK, justifyContent: 'center', position: 'absolute', left: 0, zIndex: 1000, right: 0, height: 45}}>
                        <Text style={{textAlign: 'center', color: WHITE, fontSize: 16}}>{this.props.gameEditError}</Text>
                    </View>
                );
            ...
            <View style={{flex: 1}}>
                ...
                <ScrollView 
                    ...
                    <View style={{backgroundColor: WHITE}}>
                            ...
                            {this.props.mySingleGameLoading ? <Spinner/> : null}
                            ...
                            <View style={{flexDirection: 'row', justifyContent: 'space-between', flex: 1}}>
                                <View style={{flex: 1, height: 100, marginLeft: 30}}>
                                    ...
                                    <TouchableOpacity onPress={() => this.setState({isStartTimeVisible: true})} style={{backgroundColor: LIGHT_GRAY, borderRadius: 40, padding: 10, marginRight: 10, flex: 1, height: 42}}>
                                ...
                                <View style={{flex: 1, height: 100,  marginRight: 30}}>
                                    ...
                                    <TouchableOpacity onPress={() => this.setState({isStakeEndTimeVisible: true})} style={{backgroundColor: LIGHT_GRAY, borderRadius: 40, padding: 10, marginLeft: 10, flex: 1, height: 42}}>
                            ...
                            <View style={{flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between'}}>
                                <View style={{flex: 1, marginLeft: 30}}>
                                    ...
                                    <TouchableOpacity onPress={() => this.setState({isCompletionTimeVisible: true})} style={{backgroundColor: LIGHT_GRAY, borderRadius: 40, padding: 10, marginRight: 10, flex: 1, height: 42}}>
                            ...
                            <View style={{flexDirection: 'row', marginLeft: 30, marginRight: 30, justifyContent: 'space-between'}}>
                                <View style={{flex: 1, marginRight: 5}}>
                                    <View>
                                        {
                                            this.state.sides[0] && this.state.sides[0].icon ? 
                                        ...
                                        <Item rounded style={{backgroundColor: LIGHT_GRAY, height: 42}}>
                                            <Input 
                                                ...
                                                maxLength={30}
                                                value={this.state.sides[0] ? this.state.sides[0].name : ''}
                                    ...
                                    <Item regular style={{ borderRadius: 10, backgroundColor: LIGHT_GRAY, marginTop: 10, paddingTop: 5}}>
                                        <Textarea 
                                            ...
                                            rowSpan={3}
                                            autoCorrect={false}
                                            value={this.state.sides[0] ? this.state.sides[0].description : ''}
                                ...
                                <View style={{flex: 1, marginLeft: 5}}>
                                    <View>
                                    {
                                            this.state.sides[1] && this.state.sides[1].icon ? 
                                            <View style={{alignSelf: 'center', flex: 1}}>
                                                <Thumbnail
                                        ...
                                        <View>
                                            ...
                                            <Input 
                                                ...
                                                maxLength={30}
                                                value={this.state.sides[1] ? this.state.sides[1].name : ''}
                                    ...
                                    <Item regular style={{borderRadius: 10, backgroundColor: LIGHT_GRAY, marginTop: 10, paddingTop: 5}}>
                                        <Textarea 
                                            ...
                                            rowSpan={3}
                                            autoCorrect={false}
                                            value={this.state.sides[1] ? this.state.sides[1].description : ''}
...
const mapStateToProps = (state) => {
    const {mySingleGameLoading, createGameLoading} = state.loading;
    const {categories} = state.category;
    const {mySingleGame} = state.game;
    const {createGameError, gameEditError} = state.errors;
    const {name, username, user} = state.auth;
    
    return {categories,createGameError,mySingleGame,createGameLoading, gameEditError, user, mySingleGameLoading, name, username};
};

export default connect(mapStateToProps,{ editGame, clearCreateGameError, clearGameEditError})(EditGame);
